...
public class VerifyProductHavingAddToCart_Count {
	...
	@Test
	public void countNoOfProd()
	{
		WebElement books =	driver.findElement(By.xpath("//a[contains(text(),'Books')]"));
		wait.until(ExpectedConditions.visibilityOf(books)).click();
		
		List<WebElement> addToCartList = driver.findElements(By.xpath("//input[@value='Add to cart']"));
		long countOfAddToCart = addToCartList.stream().count();
		
		System.out.println("No of add to cart buttons : "+countOfAddToCart);
	}
